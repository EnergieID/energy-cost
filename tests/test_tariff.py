...
from energy_cost.tariff import MeterType, PowerDirection, Tariff, TimedPriceFormula
...
def test_get_formulas_returns_only_overlapping_items() -> None:
    ...
    tariff = Tariff(
        supplier="S",
        product="P",
        by_meter_type={MeterType.SINGLE_RATE: {PowerDirection.CONSUMPTION: formulas}},
    )
...
def test_get_cost_raises_when_meter_type_missing() -> None:
    ...
    with pytest.raises(ValueError, match="No meters of type 'single_rate' and direction 'consumption' found in tariff"):
        tariff.get_cost(
            meter_type=MeterType.SINGLE_RATE,
            direction=PowerDirection.CONSUMPTION,
            start=dt.datetime(2025, 1, 1, 0, 0),
