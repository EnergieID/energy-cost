...
class CapacityComponent(BaseModel):
    ...
    def apply(
        ...
        if is_divisor(resolution, self.measurement_period):
            capacity_data = (
        ...
        if not is_divisor(resolution, self.billing_period):
        ...
        return self.pricing.apply(capacity_data, resolution=self.billing_period)
