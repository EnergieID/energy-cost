...
class TieredFormula(Formula):
    ...
    def apply(
        ...
        for period_start_key, group in groups:
            ...
            if self.mode == TieringMode.PROGRESSIVE:
                applied = self._apply_progressive_group(group_frame, estimated_total, resolution, timezone)
            else:
    ...
    def _apply_banded_group(
        ...
        return matched_band.formula.apply(group_frame.copy(), resolution=resolution, timezone=timezone)
    ...
    def _apply_progressive_group(
        ...
        for band in self.bands:
            ...
            if fraction > 0:
                applied = band.formula.apply(group_frame.copy(), resolution=resolution, timezone=timezone)
